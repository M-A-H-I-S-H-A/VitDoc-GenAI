document.addEventListener('DOMContentLoaded', () => {
    ...
    generateAIResponse(message, hasImage).then(response => {
    ...
    // LLM-based AI Response (FINAL CLEAN VERSION)
async function generateAIResponse(input, hasImage = false) {
    try {
        const storedUser = localStorage.getItem('vitdoc_user');
        let profile = {};
        let userContextHTML = "";
        ...
        if (storedUser) {
            ...
            if (user.profileData) {
                ...
                const diet = profile['q-diet'] || 'Not specified';
                const conds = profile['q-current-cond'] || 'None';
                const age = profile['q-age'] || 'N/A';

                userContextHTML = `<div class="user-context">
                    <i class="fas fa-user-circle"></i> 
                    Insight based on your profile: Age ${age}, Diet - ${diet}, Medical - ${Array.isArray(conds)?conds.join(', '):conds}
                </div>`;
            }
        }
        ...
        const intro = "Based on AI analysis, here are the possible deficiencies:";

        const possibilities = [
            {
                name: "AI-Based Analysis",
                match: 90,
                analysis: data.reply,
                sources: "Maintain a balanced diet and consult a doctor if needed."
            }
        ];

        const outro = "This result is AI-generated and not a medical diagnosis.";
